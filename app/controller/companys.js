...
class GoodsController extends Controller {
  ...
  async index() {
    ...
    if (page || size) {
      // 分页查询
      companysObj = await ctx.model.Companys.findAndCountAll({
      ...
      // 查询所有
      companysObj = await ctx.model.Companys.findAll();
    }
    ...
    ctx.body = {
      code: 200,
      data: {
        list: companysObj.rows,
        total: companysObj.count,
      },
    };
  }
  // 新增
  async create() {
    ...
    const haveData = await ctx.model.Goods.findAll({ where: { name } });
    ...
    const res = await ctx.model.Goods.create({ name, time, emperor, city });
    if (!res) {
  ...
  async delete() {
    ...
    // 通过主键来查询一条记录
    const user = await ctx.model.Goods.findByPk(id);
    if (!user) {
      ctx.body = {
        status: 404,
        message: "操作数据库失败",
  ...
  async findOne() {
    ...
    const { id } = ctx.query;
    // 查询单个
    const detail = await ctx.model.Goods.findOne({
      where: {
        id,
      },
      include: {
